...
from ant_colony_improve1_3 import AntColony
from improve5 import smooth_path_bspline

MAP_FILE = 'map3.txt'
RUNS = 5  # run 5 times to get average metrics
...
def run_single_config(name, use_imp1, use_imp3, map_obj):
    ...
    for r in range(RUNS):
        start_t = time.time()
        # init with feature flags
        aco = AntColony(map_obj, 50, 30, 0.5, 10.0, 
...
def main():
    ...
    # rerun once using best algorithm (imp3) to get path
    aco_best = AntColony(map_obj, 50, 30, 0.5, 10.0, use_cone_pheromone=True, use_division_of_labor=True)
