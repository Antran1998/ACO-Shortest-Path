...
\begin{itemize}
    \item \textbf{Map a}: Simple environment
    \item \textbf{Map b}: Cluttered environment
    \item \textbf{Map c}: Crowded environment
    \item \textbf{Map d}: Maze-like environment
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
    \multirow{2}{*}{map a (Simple)} & Basic ACO & 6.11 $\pm$ 0.71 & 0.112 \\
     & Adaptive (O2) & 5.24 $\pm$ 0.00 & 0.102 \\
        \hline
    \multirow{2}{*}{map b (Cluttered)} & Basic ACO & 87.67 $\pm$ 8.41 & 2.278 \\
     & Adaptive (O2) & 90.34 $\pm$ 27.88 & 2.809 \\
        \hline
    \multirow{2}{*}{map c (Crowded)} & Basic ACO & 36.67 $\pm$ 1.91 & 0.703 \\
     & Adaptive (O2) & 24.18 $\pm$ 1.60 & 0.611 \\
        \hline
    \multirow{2}{*}{map d (Maze-like)} & Basic ACO & 85.30 $\pm$ 10.49 & 2.376 \\
...
Both algorithms easily find the optimal path, but O2 converges more quickly and stably, always reaching the best solution (O2, Map a: 5.24 $\pm$ 0.00) compared to Basic ACO, which is more affected by randomness (Basic ACO, Map a: 6.11 $\pm$ 0.71).

\subsubsection*{\textbullet\ Cluttered Environment (Map b)}
In Map b, the mean path length for O2 (O2, Map b: 90.34 $\pm$ 27.88) is comparable to Basic ACO (Basic ACO, Map b: 87.67 $\pm$ 8.41), but the higher standard deviation reflects the increased difficulty and diversity of solutions in this environment. O2 still shows adaptability in avoiding local optima, but the complexity leads to more variable results.

\subsubsection*{\textbullet\ Crowded Environment (Map c)}
O2 is especially effective, almost always finding the optimal path (O2, Map c: 24.18 $\pm$ 1.60), while Basic ACO is more likely to get stuck or take detours (Basic ACO, Map c: 36.67 $\pm$ 1.91).

\subsubsection*{\textbullet\ Maze-like Environment (Map d)}
In Map d, O2 achieves a much lower mean path length (O2, Map d: 63.34 $\pm$ 13.97) than Basic ACO (Basic ACO, Map d: 85.30 $\pm$ 10.49), demonstrating a clear advantage in finding shorter paths and avoiding local optima.
