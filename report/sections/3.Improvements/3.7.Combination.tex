...
The four test maps correspond to different environments:
\begin{itemize}
    \item \textbf{Map a}: Simple
    \item \textbf{Map b}: Cluttered
    \item \textbf{Map c}: Crowded
    \item \textbf{Map d}: Maze-like
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{Map Name (Description)} & \textbf{Algorithm} & \textbf{Mean path length} & \textbf{Computational time (s)} \\
        \hline
        \multirow{8}{*}{map a (Simple)} 
        ...
        \multirow{8}{*}{map b (Cluttered)} 
            ...
            & \textbf{Proposed Method (Full)} & 52.19 $\pm$ 2.15 & 2.629 \\
        \hline
        \multirow{8}{*}{map c (Crowded)} 
            ...
            & \textbf{Proposed Method (Full)} & 23.95 $\pm$ 1.38 & 0.926 \\
        \hline
        \multirow{8}{*}{map d (Maze-like)} 
            ...
            & \textbf{Proposed Method (Full)} & 48.06 $\pm$ 2.05 & 1.503 \\
        \hline
    \end{tabular}
\end{table}
...
\subsubsection*{\textbullet\ Simple Environment (Map a)}
All improvements reach the optimal path (5.24 $\pm$ 0.00). The full combination is close to the fastest variants (O2/O1), with negligible overhead.

\subsubsection*{\textbullet\ Cluttered Environment (Map b)}
Cone initialization (O1) attains the best mean length (49.21). The combined method remains near-optimal (52.19) and far better than Basic ACO, with moderate runtime.

\subsubsection*{\textbullet\ Crowded Environment (Map c)}
O1/O3 slightly edge the best mean lengths, while the combined method is close (23.95) with acceptable time. Gains over Basic ACO are substantial.

\subsubsection*{\textbullet\ Maze-like Environment (Map d)}
The combined method is effectively tied with the best single components (O1/O4) on mean length (48.06 vs.\ 47.92--47.98) and remains near the fastest runtime.

\subsubsection*{Summary and Insights}
Across all maps, the \emph{Proposed Method (Full)} consistently improves over Basic ACO and matches or approaches the best single-component results, with notably low variance in complex maps (e.g., Map b: $\pm$2.15 vs.\ $\pm$25.78 for O6). This indicates that coordinated integration of O1--O6 provides robust, high-quality paths with competitive efficiency.